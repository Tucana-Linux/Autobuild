#!/bin/bash
source /media/EXSTOR/autobuilds/04-04-2025/autobuild.conf
...
python3 $SCRIPTS_PATH/classic-scrape-recursive.py "$URL/$LATEST_FOLDER" | grep -E "cmake-[0-9]\.[0-9]+\.[0-9]+\.tar\.gz" | sort -rV | head -1 | grep -Eo "[0-9]\.[0-9]+\.[0-9]+" &> /dev/null
if [[ $? == 1 ]]; then
  LATEST_FOLDER=$(python3 $SCRIPTS_PATH/classic-scrape-recursive.py $URL | grep [0-9] | grep '\.' | sort -rV | sed '1d' | head -1)
fi
python3 $SCRIPTS_PATH/classic-scrape-recursive.py "$URL/$LATEST_FOLDER" | grep -E "cmake-[0-9]\.[0-9]+\.[0-9]+\.tar\.gz" | sort -rV | head -1 | grep -Eo "[0-9]\.[0-9]+\.[0-9]+"
